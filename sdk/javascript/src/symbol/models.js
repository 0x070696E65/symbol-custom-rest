...
class TransfervTransaction {
	static TRANSACTION_VERSION = 1;

	static TRANSACTION_TYPE = TransactionType.TRANSFERV;
	...
	constructor() {
		this._signature = new Signature();
		this._signerPublicKey = new PublicKey();
		this._version = TransfervTransaction.TRANSACTION_VERSION;
		this._network = NetworkType.MAINNET;
		this._type = TransfervTransaction.TRANSACTION_TYPE;
		this._fee = new Amount();
		this._deadline = new Timestamp();
		this._recipientAddress = new UnresolvedAddress();
		this._mosaics = [];
		...
		this._verifiableEntityHeaderReserved_1 = 0; // reserved field
		this._entityBodyReserved_1 = 0; // reserved field
		this._transfervTransactionBodyReserved_1 = 0; // reserved field
		this._transfervTransactionBodyReserved_2 = 0; // reserved field
	}
	...
	static deserialize(payload) {
		...
		const mosaicsCount = converter.bytesToInt(view.buffer, 1, false);
		view.shiftRight(1);
		const transfervTransactionBodyReserved_1 = converter.bytesToInt(view.buffer, 1, false);
		view.shiftRight(1);
		if (0 !== transfervTransactionBodyReserved_1)
			throw RangeError(`Invalid value of reserved field (${transfervTransactionBodyReserved_1})`);
		const transfervTransactionBodyReserved_2 = converter.bytesToInt(view.buffer, 4, false);
		view.shiftRight(4);
		if (0 !== transfervTransactionBodyReserved_2)
			throw RangeError(`Invalid value of reserved field (${transfervTransactionBodyReserved_2})`);
		const mosaics = arrayHelpers.readArrayCount(view.buffer, UnresolvedMosaic, mosaicsCount, e => e.mosaicId.value);
		view.shiftRight(arrayHelpers.size(mosaics));
		const message = new Uint8Array(view.buffer.buffer, view.buffer.byteOffset, messageSize);
		view.shiftRight(messageSize);

		const instance = new TransfervTransaction();
		instance._signature = signature;
		instance._signerPublicKey = signerPublicKey;
		instance._version = version;
		instance._network = network;
	...
	serialize() {
		...
		buffer.write(converter.intToBytes(this._message.length, 2, false)); // bound: message_size
		buffer.write(converter.intToBytes(this._mosaics.length, 1, false)); // bound: mosaics_count
		buffer.write(converter.intToBytes(this._transfervTransactionBodyReserved_1, 1, false));
		buffer.write(converter.intToBytes(this._transfervTransactionBodyReserved_2, 4, false));
		arrayHelpers.writeArray(buffer, this._mosaics, e => e.mosaicId.value);
		buffer.write(this._message);
		return buffer.storage;
	}
...
class EmbeddedTransfervTransaction {
	static TRANSACTION_VERSION = 1;
	...
	constructor() {
		this._signerPublicKey = new PublicKey();
		this._version = EmbeddedTransfervTransaction.TRANSACTION_VERSION;
		this._network = NetworkType.MAINNET;
		this._type = EmbeddedTransfervTransaction.TRANSACTION_TYPE;
		this._recipientAddress = new UnresolvedAddress();
		this._mosaics = [];
		this._message = new Uint8Array();
		this._embeddedTransactionHeaderReserved_1 = 0; // reserved field
	...
	static deserialize(payload) {
		...
		const instance = new EmbeddedTransfervTransaction();
		instance._signerPublicKey = signerPublicKey;
		instance._version = version;
		instance._network = network;
		instance._type = type;
